...
def frac_repetitions(n, u=0.1):
    """Computes significance repetition requirement at each decision point that is a constant
    ...
    """
    return np.ceil(np.arange(1, 1 + n) * u)

def geo_budget(n, w):
    """Computes budget allocation for n out of infinity decision points using a fraction proportional
    to :math:w \cdot w^(t - 1):math.
